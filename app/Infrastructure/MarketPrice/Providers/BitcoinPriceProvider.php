<?php

namespace App\Infrastructure\MarketPrice\Providers;

use App\Application\MarketPrice\Contracts\MarketPriceProviderInterface;
use App\Application\MarketPrice\DTOs\MarketPriceRecord;

class BitcoinPriceProvider extends AbstractHybridProvider implements MarketPriceProviderInterface
{
    public function key(): string
    {
        return 'bitcoin';
    }

    public function fetch(): array
    {
        $config = config('services.market_data.providers.bitcoin', []);
        $quotedAt = new \DateTimeImmutable();
        $price = null;
        $sourceType = 'api';

        $json = $this->loadJson($config['api_url'] ?? null);
        $price = $this->toFloat(data_get($json, 'bitcoin.usd'));

        if ($price === null) {
            $sourceType = 'scrape';
            $price = $this->numberFromHtml($this->loadHtml($config['scrape_url'] ?? null));
        }

        if ($price === null) {
            return [];
        }

        return [
            new MarketPriceRecord(
                instrument: 'BTCUSD',
                category: 'crypto',
                value: $price,
                currency: 'USD',
                unit: '1 BTC',
                quotedAt: $quotedAt,
                source: (string) ($config['name'] ?? 'bitcoin'),
                sourceType: $sourceType,
            ),
        ];
    }
}
